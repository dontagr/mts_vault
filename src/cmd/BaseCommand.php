...
class BaseCommand extends Command
{
    ...
    {
        $result = [];
        exec(sprintf('consul kv get -keys "variables/dev/shopdev_front/" 2> %s/../../logs/errors.log', __DIR__), $result);
        if (!$result) {
    ...
    protected function getValues(string $secret, string $env, Data $vaultData): array
    {
        $this->logger->info(sprintf('Получаю данные для env=%s', $env));
        $response = $vaultData->get($this->getVaultPathName($secret, $env));

        return json_decode((string)$response->getBody(), true, 512, JSON_THROW_ON_ERROR)['data']['data'] ?? [];
    }

    protected function writeValues(string $secret, string $env, Data $vaultData, $values): bool
    {
        $response = $vaultData->write($this->getVaultPathName($secret, $env), ['data' => $values]);

        return $response->getStatusCode() === 200;
    }

    protected function getVaultPathName(string $secret, string $env): string
    {
        return sprintf('%s/data/%s/%s', $_ENV['VAULT_WORK_SPACE'], $secret, $env);
    }
}